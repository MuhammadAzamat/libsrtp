...
srtp_err_t gost_encrypt_func(void *state, srtp_stream_ctx_t *stream, unsigned char *buf, unsigned int *enc_len) {
   gost_key_t key;
   gost_ctx_t *ctx;
   unsigned char iv[8];
   ...
   // Set key
   key.k = stream->gost_key;
   ctx = gost_create(&key);

   // Generate random IV
   if (stream->session_key_len <= sizeof(iv)) {
       srtp_random(iv, stream->session_key_len);
   } else {
       srtp_random(iv, sizeof(iv));
   }

   // Encrypt
   err = gost_mac(ctx, iv, buf, *enc_len, &stream->rtcp_pkt_ctx[0].gost_mac_tag, &tag_len);

   if (err != srtp_err_status_ok)
       return err;

   // Append IV to ciphertext
   memcpy(buf + *enc_len, iv, sizeof(iv));
   *enc_len += sizeof(iv);
...
srtp_err_t gost_decrypt_func(void *state, srtp_stream_ctx_t *stream, unsigned char *buf, unsigned int *enc_len) {
   gost_key_t key;
   gost_ctx_t *ctx;
   unsigned char iv[8];
   unsigned int tag_len;
   ...
   *enc_len -= sizeof(iv);
   memcpy(iv, buf + *enc_len, sizeof(iv));
...
srtp_err_t gost_crypto_init(srtp_crypto_policy_t *policy, srtp_stream_ctx_t *stream) {
   if (policy->cipher_type != GOST_CIPHER) {
       return srtp_err_status_bad_param;
   }

   stream->gost_key = policy->gost_key;

   stream->rtp_cipher->type = GOST_CIPHER;
   stream->rtp_cipher->key_len = policy->gost_key_len;
   stream->rtp_cipher->iv_len = 8;
   stream->rtp_cipher->encrypt = gost_encrypt_func;
   stream->rtp_cipher->decrypt = gost_decrypt_func;

   stream->rtcp_cipher->type = GOST_CIPHER;
   stream->rtcp_cipher->key_len = policy->gost_key_len;
   stream->rtcp_cipher->iv_len = 8;
   stream->rtcp_cipher->encrypt = gost_encrypt_func;
   stream->rtcp_cipher->decrypt = gost_decrypt_func;
